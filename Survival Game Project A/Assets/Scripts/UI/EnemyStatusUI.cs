...
public class EnemyStatusUI : MonoBehaviour
{
    ...
    private void Awake()
    {
        // Tự động ẩn toàn bộ các icon khi khởi chạy
    ...
    private void Update()
    {
        ...
        foreach (var config in statusConfigurations)
        {
            if (string.IsNullOrEmpty(config.effectClassName)) continue;

            // Kiểm tra xem controller của quái có đang dính hiệu ứng này không thông qua Reflection tên class
            if (controller.TryGetEffectDuration(Type.GetType(config.effectClassName), out float remain, out float max))
            {
